<?php
  ...
  if(!empty($_POST)){
    //DBに会員情報を登録
    $sql = sprintf('INSERT INTO `sirumoku_entry` SET `event_date` = "%s", `application_date` = "%s", `student_number` = "%s", `opinion` = "%s"',
  ...
  //都道府県情報の取得
  $sql_pref = sprintf('SELECT * FROM `prefectures` WHERE `prefecture_id` = "%d"', $_SESSION['join']['pref']);
  $record_pref = mysqli_query($db, $sql_pref);
  $pref = mysqli_fetch_assoc($record_pref);
  //シルモク開催情報の取得
  $sql_sirumoku = sprintf('SELECT * FROM `sirumoku_data` WHERE `date` = "%s"', $date);
  ...
  //学科情報の取得
  $sql_department = sprintf('SELECT * FROM `departments` WHERE `department_id` = "%d"', $department);
  $record_department = mysqli_query($db, $sql_department);
  $department = mysqli_fetch_assoc($record_department);
?>
<!DOCTYPE html>
<html>
  <head>
          ...
          <table class="table table-striped table-condensed">
            <tbody>
              ...
              <tr>
                ...
                <td><div class="text-center"><?php echo htmlspecialchars($department['department_name']); ?></div></td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td><div class="text-center"><?php echo htmlspecialchars($pref['prefecture_name']); ?></div></td>
              </tr>
            </tbody>
          </table>
